...
```bash
uv run dedup-photos plan /local/batch/google-photos.manifest.csv /local/batch/backups.manifest.csv \
...
Manifest mode uses file size plus `xxh128` for primary image identity. Sidecar rows are used for keeper precedence and for moving duplicate bundles. Optional byte-level verification rereads NAS primary and sidecar files that share a same-size/same-hash manifest bucket:

```bash
uv run dedup-photos verify-bytes /local/batch/google-photos.manifest.csv /local/batch/backups.manifest.csv \
...
```bash
uv run dedup-photos verify-move /local/batch/google-photos.manifest.csv /local/batch/backups.manifest.csv \
  --output /volume1/photo/dupes \
